(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["v-status"] = factory(require("vue"));
	else
		root["v-status"] = factory(root["Vue"]);
  ...
  return {
    computed: _defineProperty({}, props.name, {
      get: function get() {
         ...
         */
        if (!this._vStatusMixin) // create a Proxy of Observable Vue object if not exists
          this._vStatusMixin = new Proxy(external_commonjs_vue_commonjs2_vue_root_Vue_default.a.observable({}), {
            // get generate status objects
            get: function get(obj, attr) {
            ...
            set: function set(obj, attr, promise) {
              ...
              if (Array.isArray(promise)) promise = Promise.all(_toConsumableArray(promise.map(function (p) {
                return p.catch(function (err) {
                  current[attr].errors.push({
                    message: Object.is(err) ? err.message : err
                  });
                });
              })));
              ...
              if (!promise.then) // check if is a valid promise, raise an error if not
                throw 'need to be a promise'; // clear and change state to loading
              ...
              promise.then(function (res) {
                if (!current[attr].errors.length) {
                  ...
                  current[attr].error.message = "".concat(current[attr].errors.length, " error(s)");
                }
...
//# sourceMappingURL=v-status.umd.js.map